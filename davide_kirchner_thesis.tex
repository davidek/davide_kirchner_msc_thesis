...
%\documentclass[10pt,a4paper,twoside,titlepage,openany]{book}  % review draft print

%\usepackage[outer=1.5cm,inner=2.5cm,top=2cm,bottom=2cm]{geometry}  % suggested by DISI template
%\usepackage[outer=1.5cm,inner=1.5cm,bottom=1.5cm,top=1.5cm]{geometry}  % review draft print
\usepackage[outer=3.5cm,inner=3.5cm,bottom=4cm,left=3.5cm,right=3.5cm]{geometry}
...
\usepackage[nottoc,chapter,notlof,notlot]{tocbibind} % ToC item for Bibliography at chapter level [no ToC item for ToC]
...
\currentpdfbookmark{\contentsname}{\contentsname}  % add toc to pdf bookmarks but not to toc
\tableofcontents
...
When multiple CPU sockets are installed on a server, it is common that separate memory banks are directly attached to each one, resulting in a \glsfirst{NUMA} architecture: each processing core has a direct access to memory allocated on a bank on the same socket (called NUMA \emph{node}), while access to ``remote'' memory is slower and may cause extra bus conflicts.
...
%todo[inline]{magari si vedrebbe ancora meglio se l'echo server perdesse dei pacchetti a caso (tipo, in media due al secondo)}
...
In this work, I presented the first results of the performance evaluation for Augustus, a software routing application for \glsfirst{CCN} based on DPDK for packet I/O.
...
\lstinputlisting[float=bt, captionpos=b, label=lst:augustus.click, frame=lines, caption=Sample Click configuration excerpt using the API under development]{extra/icnrouter.click.txt}
...
\renewcommand{\listofalgorithms}{\@starttoc{loa}}
